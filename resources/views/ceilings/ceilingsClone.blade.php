@extends('common.layoutClone', ['seoData' => $seoData])
...
@section('content')
    ...
    <section class="manufacturers manufacturers_main manufacturers_ceilings container">
        ...
        <p class="manufacturers__text">Компания &laquo;Твой стиль&raquo; ипользует плёнку для натяжных потолков ведущих мировых производителей. Мы&nbsp;предлагаем нашим клиентам только сертифицированные материалы. Работаем с&nbsp;ПВХ-плёнкой и&nbsp;тканевыми полотнами следующих производителей:</p>
	...
	<section class="seo-block seo-block_main seo-block_ceilings"
        @if (!empty($seoData->background_mobile)) data-backgr-mobile="url({{ Storage::url($seoData->background_mobile) }})" @endif
        @if (!empty($seoData->background_tablet)) data-backgr-tablet="url({{ Storage::url($seoData->background_tablet) }})" @endif
        @if (!empty($seoData->background_image)) style="background-image: url({{ Storage::url($seoData->background_image) }})" @endif
    >
