<template>
	<div>
		<section class="quiz">
			<p class="overtitle quiz__overtitle">Тест займёт 15 секунд</p>
	        ...
	        <a :href="cityLink" class="button quiz__button" style="margin-top: 16px">Перейти на сайт</a>
	        ...
	        <div v-show="opened" class="quiz__popup">
			    ...
			    <div v-if="step < 8" class="quiz__titles"> 
			    	<p v-if="step < 4" class="overtitle quiz__step">Вопрос {{ step }}/3</p>
			    	<p v-if="step == 5" class="overtitle quiz__step">Спасибо за ответы!</p>
			    	<p v-if="step == 1" class="second-title">Укажите примерный метраж помещения?</p>
			    	<p v-if="step == 2" class="second-title">Какое хотели бы освещение в потолке?</p>
			    	<p v-if="step == 3" class="second-title">Когда планируете делать потолок?</p>
			    	<p v-if="step == 4" class="second-title">Выберите ваш подарок к потолку</p>
			    	<p v-if="step == 5" class="second-title">Получите расчет потолка и приятный подарок</p>
			    </div>
			    <div v-if="step == 1" class="quiz__content quiz__content_area">
			    	...
			    	<label class="radio quiz__radio">
			    		<input v-model="questions.area" value="от 30" type="radio" class="radio__input">
			    		<i class="radio__mark"></i>
			    		<span class="radio__label">От 30 кв.м</span>
			    	</label>
			    </div>
			    ...
			    <div v-if="step == 3" class="quiz__content quiz__content_when">
			    	<label class="radio quiz__radio">
			    ...
			    <div v-show="step == 5" class="quiz__content quiz__result">  
		    		<p class="text">Введите номер телефона и вы получите расчет на WhatsApp и СМС в течении 5&nbspминут</p>
		    		<input autofocus="true" v-mask="{mask: '8 999-999-99-99', oncomplete: enable, onincomplete: disable}" v-model="questions.phone" class="input quiz__phone" :class="{'input_invalid': !phoneValidated}" type="tel" placeholder="Ваш телефон*">
		    		<button :disabled="!phoneValidated" @click="send" class="button quiz__button">Получить результат расчёта</button>
			    </div> 
			    <div v-if="step < 5" class="quiz__buttons">
			    ...
			    <div v-if="step == 6" class="quiz__success">
			    	<p class="overtitle quiz__step">Спасибо!</p>
			    	<p class="second-title">Заявка принята</p>
			    	<a :href="cityLink" class="button">Перейти на сайт</a>
			    </div>
	        </div>
	    </section>
